...
class MapRenderer:
    def __init__(self, engine):
        ...
        self.vertices = self.wad_data.vertices
        self.linedefs = self.wad_data.linedefs
        self.x_min, self.x_max, self.y_min, self.y_max = self.get_map_bounds()
        #remap to fit the screen
        self.vertices = [
            pg.Vector2(self.remap_x(v.x), self.remap_y(v.y)) for v in self.vertices
        ]
    ...
    def draw_seg(self, seg, sub_sector_id):
        v1 = self.vertices[seg.start_vertex_id]
        v2 = self.vertices[seg.end_vertex_id]
        pg.draw.line(self.engine.screen, 'green', v1, v2, 4)

    def draw_linedefs(self):
        for line in self.linedefs:
            p1 = self.vertices[line.start_vertex_id]
            p2 = self.vertices[line.end_vertex_id]
    ...
    def remap_y(self, n, out_min=30, out_max=HEIGHT-30):
        return out_max - (
            (max(self.y_min, min(n, self.y_max)) - self.y_min)
            * (out_max - out_min) / (self.y_max - self.y_min)
        )
    ...
    def get_map_bounds(self):
        ...
        y_sorted = sorted(self.vertices, key=lambda v: v.y)
        y_min = y_sorted[0].y
        y_max = y_sorted[-1].y
