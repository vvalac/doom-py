...
class SegHandler:
    ...
    def draw_portal_wall_range(self, x1, x2):
        ...
        if b_draw_upper_wall:
            if world_back_z2 > world_front_z2:
        ...
        for x in range(x1, x2 + 1):
            ...
            if b_draw_ceil:
                cy1 = upper_clip[x] + 1
                cy2 = int(min(draw_upper_wall_y1 - 1, lower_clip[x] - 1))
                renderer.draw_vline(x, cy1, cy2, tex_ceil_id, light_level)
    ...
    def classify_segment(self, segment, x1, x2, rw_angle1):
        ...
        # reject empty lines for triggers
        # cover floor and ceiling on both sides, identical
        # light levels on both sides, no middle tex
